...
def get_raa(url, sunsu):
    ...
    for sc in score:
        #     print(sc.text)
        if sc.text == sunsu:  # 요기 바꾸면 선수 아무나 크롤링가능!
            raa.append(sc.text)
            start = 1
        if start > 0:
            start += 1
...
if __name__ == '__main__':
    ...
    sunsu_list = ['권희동']
    year = 2014
    for sunsu in sunsu_list:
        url = 'http://www.statiz.co.kr/player.php?opt=6&sopt=0&name=%s&re=0&da=11&year=%s&plist=&pdate=' % (
            sunsu, year)
        url2 = 'http://www.statiz.co.kr/stat.php?opt=0&sopt=0&re=0&ys=%s&ye=%s&se=0&te=&tm=&ty=0&qu=auto&po=0&as=&ae=&hi=&un=&pl=&da=11&o1=RE24_BR2&o2=WAR_ALL&de=1&lr=0&tr=&cv=&ml=1&sn=30&si=&cn=' % (
            year, year)
        df = get_SB_data(url)
        dk = get_raa(url2, sunsu)
        da = merge_raa_SB_data(dk, df)
        print(da)
        da.to_csv('%s.csv' % sunsu, encoding='euc-kr')
